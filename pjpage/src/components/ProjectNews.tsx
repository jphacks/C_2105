...
export const ProjectNews: FC<Props> = ({ id }) => {
  const useQueryColumns = () => {
    const getColumns = async (): Promise<Column[]> => {
      const { data } = await axios.get<Column[]>(
        `${process.env.REACT_APP_REST_URL}column`
      );
      return data;
    };
  ...
  const { data } = useQueryColumns();
  let latestNews = data.reverse().filter(dt => dt.id === id);
    ...
    <div className="has-text-left my-5">
      ...
      { latestNews.map((data) => (
        <div className="columns is-mobile card m-2">
          <img className="column is-3" src={data.imgUrl} alt={data.columnTitle} />
          <div className="column is-9 is-relative">
            <h3 className="is-size-5 has-text-weight-semibold">{data.columnTitle}</h3>
